...
bun -e '
  ...
  p.scripts = { ...p.scripts, web: "dsh web", cli: "dsh-tui", headless: "dsh --profile headless", "sync-models": "bun run sync-models.js", ...(fs.existsSync("doctor.js") ? { doctor: "bun doctor.js" } : {}), ...(fs.existsSync("apply-template.js") ? { template: "bun apply-template.js" } : {}), ...(fs.existsSync("reset.sh") ? { reset: "bash reset.sh" } : {}) };
...
echo "➡️ To switch personas & activate skills:       bun run template"
